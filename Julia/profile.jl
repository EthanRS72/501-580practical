...
# ╔═╡ c7d45060-a770-11ee-3123-677796628136
using CSV, DataFrames, Statistics, Plots, MLJ, StatsBase, Imbalance, StatsPlots, InformationMeasures, PyCall, Pkg

# ╔═╡ b8264f6c-766e-4c4c-9a70-6626eeb5b5b2
begin
	df = CSV.read("contributors.csv", DataFrame; header = true)
end

# ╔═╡ c7a81fe6-1c8c-4284-b008-d36d8f399faa
df2 = df[:, [10, 29, 30, 31, 32, 34, 35]]

# ╔═╡ db1987a4-859a-41b4-bf98-01c82bd72dfb
countmap(df2.is_core)

# ╔═╡ 7203b171-0c56-4afe-a6b3-2be8f0f8f4b0
function corrplot(data::DataFrame)
	cor_mat = cor(Matrix(data))
	heatmap(cor_mat,
		c=:lighttest, title="Correlation Plot",
		xticks= (1:size(cor_mat, 1), names(data)),
		yticks = (1:size(cor_mat, 2), names(data)),
		xrotation = 45)
	
	fontsize = 8
	nrow, ncol = size(cor_mat)
	ann = [(i,j, text(round(cor_mat[i,j], digits=2), fontsize, :black, :center))
            for i in 1:nrow for j in 1:ncol]
	annotate!(ann, linecolor=:white)

end

# ╔═╡ 327b7b71-75e2-4386-9633-b8921a35c79c
corrplot(df2)

# ╔═╡ da0880d7-7bbf-4e93-9a1c-8be1d8e90da8
function oversample_data_binary(data::DataFrame, minority_class::Float64, majority_class::Float64)
	
	# Count the number of samples in the minority class
	num_minority_samples = count(x -> x == minority_class, data.is_core)

	# Count the number of samples in the majority class
	num_majority_samples = count(x -> x == majority_class, data.is_core)
	
	# Find all indices of the minority class
	minority_indices = findall(x -> x == minority_class, data.is_core)
	# Use sampling with replacement to generate enough samples to balance the data
	oversampled_indices = sample(minority_indices, num_majority_samples - num_minority_samples, replace=true)
	
	# Add the random sample to the orginal data
	oversampled_data = vcat(data, data[oversampled_indices, :])
	return oversampled_data

end

# ╔═╡ 60bed44c-71d6-40d6-a942-1bacfd6c0b50
df2_new = oversample_data_binary(df2, 1.0,0.0)

# ╔═╡ 2bd548c4-7783-479f-8e1c-9ccb76da6495
begin
	
	# Assuming df2 is your DataFrame with columns "months_experience" and "is_core"
	plot(df2_new.months_experience, df2_new.followers, color = df2_new.is_core, seriestype=:scatter, legend=false)
end

# ╔═╡ ff01482b-632e-417a-8332-058201201b2e
filtered_0 = filter(row -> row.is_core == 0, df2)

# ╔═╡ fc62b78e-3a89-4602-ab96-653f66d09ca5
filtered_1 = filter(row -> row.is_core == 1, df2)

# ╔═╡ 51cd0812-9596-497c-8cad-dc03d65ae3db
mean(filtered_0.months_experience)

# ╔═╡ 5ced6ad7-d8d2-4203-babb-76c711688caa
mean(filtered_1.months_experience)

# ╔═╡ 5b9d1955-eeb7-488c-8127-889a2413bc3a
boxplot([filtered_0.repos, filtered_1.repos])

# ╔═╡ 3b4a27e9-b56e-4180-a1ae-63f4541714df
boxplot([filtered_0.gists, filtered_1.gists])

# ╔═╡ c5baee06-0c1f-4153-bfb2-34b8ec390cce
boxplot([filtered_0.followers, filtered_1.followers])

# ╔═╡ c46c2aa3-f642-45c8-9348-b7b0001c70e1
boxplot([filtered_0.following, filtered_1.following])

# ╔═╡ 614ca4e4-3c08-49ab-a3d4-bbf21447ed58
boxplot([filtered_0.months_experience, filtered_1.months_experience])

# ╔═╡ 538d1378-34ed-4771-bbed-7721580caa52
begin
	d_mi_cat = select(df2_new, :is_core, :has_bio)
	
	col_names_cat = ["is_core", "has_bio"]

	onehot_encoder_mi = machine(OneHotEncoder(), d_mi_cat)
	MLJ.fit!(onehot_encoder_mi)
	df_mi_cat = MLJ.transform(onehot_encoder_mi, d_mi_cat)

	for col_name in names(df2_new)
		if col_name in names(df_mi_cat)
			df2_new[!, col_name] = df_mi_cat[!, col_name]
		end
	end

		
	mi_list = []
	mi_names = ["repos", "gists", "followers", "following", "months_experience", "has_bio"]

	y = Vector(df2_new[!,:is_core])
	for i in 1:length(mi_names)
		x = Vector(df2_new[!,i])
		mi = get_mutual_information(x,y)
		push!(mi_list, mi)
	end
	
	# Get the permutation indices to sort list1
	p = sortperm(mi_list)
	perm_indices = reverse(p)
	
	# Sort both lists using the permutation indices
	sorted_list1 = mi_names[perm_indices]
	sorted_list2 = mi_list[perm_indices]

	for i in 1:length(sorted_list1)
		print("MI of " * sorted_list1[i] * ": " * string(sorted_list2[i]) * "\n")
	end

end

# ╔═╡ c5dcbe97-f734-4b3a-83e1-2b21cb9f7801
imblearn = pyimport("imblearn")

# ╔═╡ 4f19ebb3-44c6-4fe0-829c-0ae8f692a10c
...
julia_version = "1.9.2"
manifest_format = "2.0"
project_hash = "7e2fa92521e67e09afad57de198609e19d1b0b9d"
...
git-tree-sha1 = "2cdff31d45d48b8001420dc1f6f20a36ad98dd8a"
...
git-tree-sha1 = "0a4167e43dcd96ad293fe0bded5923703c169553"
...
deps = ["AbstractTrees", "CategoricalArrays", "ColumnSelectors", "Distributions", "LinearAlgebra", "NelderMead", "PrettyTables", "Random", "ScientificTypes", "Statistics", "StatsBase", "Tables", "Transducers", "TransformsBase", "Unitful"]
git-tree-sha1 = "30e377e824e5c5e8a91cbaf2f001e974a13c1bea"
...
[[deps.TransformsBase]]
deps = ["AbstractTrees"]
git-tree-sha1 = "53e92e907bd67eef12e319ca932a7dd036428bfc"
